...
class SaleRepository extends SaleInterface {
  ...
  /**
   * Update a sale with raw MongoDB operators
   * 
   * @param {string} id - ID of the sale to update
  ...
  async updateRaw(id, updateData, transaction = null) {
    try {
      ...
      if (provider.updateRaw) {
        return await provider.updateRaw("sales", id, updateData, transaction);
      }
      ...
      console.error(`Error in raw update for sale ${id}:`, error);
      throw error;
    }
  }
}
