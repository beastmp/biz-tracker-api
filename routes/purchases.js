...
router.get('/:id', async (req, res) => {
  try {
    const purchase = await Purchase.findById(req.params.id).populate('items.item');
    ...
    if (req.body.items) {
      ...
      originalPurchase.items.forEach(item => {
        if (item.item.toString() in originalItems) {
          if (item.quantity) originalItems.get(item.item.toString()).quantity += item.quantity;
          if (item.weight) originalItems.get(item.item.toString()).weight += item.weight;
        } else {
      ...
      req.body.items.forEach(item => {
        if (item.item.toString() in newItems) {
          if (item.quantity) newItems.get(item.item.toString()).quantity += item.quantity;
          if (item.weight) newItems.get(item.item.toString()).weight += item.weight;
        } else {
      ...
      for (const itemId of allItemIds) {
        const originalVals = originalItems.get(itemId) || { quantity: 0, weight: 0 };
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    // Revert inventory quantities for each item in the purchase if status is "received"
